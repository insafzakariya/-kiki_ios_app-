//
//  SongsTileLibrarySongsSeeAll.swift
//  SusilaMobile
//
//  Created by Kiroshan on 2/26/20.
//  Copyright © 2020 Isuru Jayathissa. All rights reserved.
//

import Foundation

class SongsTileLibrarySongsSeeAll: UIView {
    ...
    var imageURL = "" {
        ...
        didSet{
            //image.downloadImageSongsTileLibrarySongsSeeAll(from: URL(string: imageURL)!)
        }
    }
    ...
    private func commonInit() {
        ...
        add = UIButton(frame: CGRect(x:  UIScreen.main.bounds.width-120, y: image.frame.height/2-10, width: 60, height: 20))
...
extension UIImageView{
    func getDataSongsTileLibrarySongsSeeAll(from url: URL, completion: @escaping (Data?, URLResponse?, Error?) -> ()) {
        URLSession.shared.dataTask(with: url, completionHandler: completion).resume()
    }
    func downloadImageSongsTileLibrarySongsSeeAll(from url: URL) {
        getDataSongsTileLibrarySongsSeeAll(from: url) { data, response, error in
