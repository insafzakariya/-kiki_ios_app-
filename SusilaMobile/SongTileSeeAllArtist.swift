...
class SongTileSeeAllArtist: UIView {
    ...
    var imageURL = "" {
        ...
        didSet{
            //image.downloadImageSongTileSeeAllArtist(from: URL(string: imageURL)!)
        }
    }
    ...
    private func commonInit() {
        ...
        lblTitle = UILabel(frame: CGRect(x: image.frame.width+10, y: 10, width: self.frame.width-20-UIScreen.main.bounds.width/6 , height: 20))
...
extension UIImageView{
    func getDataSongTileSeeAllArtist(from url: URL, completion: @escaping (Data?, URLResponse?, Error?) -> ()) {
        URLSession.shared.dataTask(with: url, completionHandler: completion).resume()
    }
    func downloadImageSongTileSeeAllArtist(from url: URL) {
        getDataSongTileSeeAllArtist(from: url) { data, response, error in
