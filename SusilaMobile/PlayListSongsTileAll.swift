...
class PlayListSongsTileAll: UIView {
    ...
    private func commonInit() {
        ...
        lblTitle = UILabel(frame: CGRect(x: image.frame.width+10, y: 0, width: self.frame.width-(UIScreen.main.bounds.width/6+85) , height: 20))
        lblDescription = UILabel(frame: CGRect(x: image.frame.width+10, y: lblTitle.frame.height, width: self.frame.width-(UIScreen.main.bounds.width/6+85), height: 20))
...
extension UIImageView{
    func getDataPlayListSongsTileAll(from url: URL, completion: @escaping (Data?, URLResponse?, Error?) -> ()) {
        URLSession.shared.dataTask(with: url, completionHandler: completion).resume()
    }
    func downloadImagePlayListSongsTileAll(from url: URL) {
        getDataPlayListSongsTileAll(from: url) { data, response, error in
