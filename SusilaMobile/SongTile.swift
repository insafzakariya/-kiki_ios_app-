...
class SongTile: UIView {
    var imageContainer = UIView(frame: CGRect(x: 0, y: 0, width: 200, height: 200))
    var image = UIImageView(frame: CGRect(x: 20, y: 20, width: 160, height: 160))
    ...
    private func commonInit(){
        ...
        image = UIImageView(frame: CGRect(x: 0, y: 0, width: (UIScreen.main.bounds.width)*2/3, height: (UIScreen.main.bounds.width)*2/3))
        ...
        image.frame = CGRect(x: 0, y: 0, width: (UIScreen.main.bounds.width)*2/3, height: (UIScreen.main.bounds.width)*2/3)
...
extension UIImageView {
    func getDataSongTile(from url: URL, completion: @escaping (Data?, URLResponse?, Error?) -> ()) {
        URLSession.shared.dataTask(with: url, completionHandler: completion).resume()
    }
    func downloadImageSongTile(from url: URL) {
        getDataSongTile(from: url) { data, response, error in
