...
class SongTileBrowsePlaylist: UIView {
    ...
    var imageURL = "" {
        ...
        didSet{
            //image.downloadImageSongTileBrowsePlaylist(from: URL(string: imageURL)!)
        }
    }
...
extension UIImageView{
    func getDataSongTileBrowsePlaylist(from url: URL, completion: @escaping (Data?, URLResponse?, Error?) -> ()) {
        URLSession.shared.dataTask(with: url, completionHandler: completion).resume()
    }
    func downloadImageSongTileBrowsePlaylist(from url: URL) {
        getDataSongTileBrowsePlaylist(from: url) { data, response, error in
