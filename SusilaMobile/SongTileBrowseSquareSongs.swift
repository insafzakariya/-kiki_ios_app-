//
//  SongTileBrowseSquareSongs.swift
//  SusilaMobile
//
...
class SongTileBrowseSquareSongs: UIView {
    ...
    var imageURL = "" {
        ...
        didSet{
            //image.downloadImageSongTileBrowseSquareSongs(from: URL(string: imageURL)!)
        }
    }
...
extension UIImageView{
    func getDataSongTileBrowseSquareSongs(from url: URL, completion: @escaping (Data?, URLResponse?, Error?) -> ()) {
        URLSession.shared.dataTask(with: url, completionHandler: completion).resume()
    }
    func downloadImageSongTileBrowseSquareSongs(from url: URL) {
        getDataSongTileBrowseSquareSongs(from: url) { data, response, error in
