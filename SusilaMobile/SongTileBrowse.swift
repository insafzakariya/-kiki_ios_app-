...
class SongTileBrowse: UIView {
    ...
    var imageURL = "" {
        ...
        didSet{
            //image.downloadImageSongTileBrowse(from: URL(string: imageURL)!)
        }
    }
    ...
    private func commonInit() {
        ...
        image = UIImageView(frame: CGRect(x: 0, y: 0, width: (UIScreen.main.bounds.width-40)*1/3, height: (UIScreen.main.bounds.width-40)*1/3))
        lblTitle = UILabel(frame: CGRect(x: 0, y: image.frame.height, width: self.frame.width-20 , height: 20))
        lblDescription = UILabel(frame: CGRect(x: 0, y: image.frame.height+lblTitle.frame.height, width: self.frame.width-20, height: 10))
        
        let img : UIImage = UIImage(named:"logo_grayscale")!
        image = UIImageView(image: img)
        image.frame = CGRect(x: 0, y: 0, width: (UIScreen.main.bounds.width-40)*1/3, height: (UIScreen.main.bounds.width-40)*1/3)
        image.layer.cornerRadius = ((UIScreen.main.bounds.width-40)*1/3)/2
...
extension UIImageView{
    func getDataSongTileBrowse(from url: URL, completion: @escaping (Data?, URLResponse?, Error?) -> ()) {
        URLSession.shared.dataTask(with: url, completionHandler: completion).resume()
    }
    func downloadImageSongTileBrowse(from url: URL) {
        getDataSongTileBrowse(from: url) { data, response, error in
