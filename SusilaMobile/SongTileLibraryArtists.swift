...
class SongTileLibraryArtists: UIView {
    ...
    var imageURL = "" {
        ...
        didSet{
            //image.downloadImageSongTileLibraryArtists(from: URL(string: imageURL)!)
        }
    }
...
extension UIImageView{
    func getDataSongTileLibraryArtists(from url: URL, completion: @escaping (Data?, URLResponse?, Error?) -> ()) {
        URLSession.shared.dataTask(with: url, completionHandler: completion).resume()
    }
    func downloadImageSongTileLibraryArtists(from url: URL) {
        getDataSongTileLibraryArtists(from: url) { data, response, error in
