...
class SongTileBrowseSquare: UIView {
    ...
    var imageURL = "" {
        ...
        didSet{
            //image.downloadImageSongTileBrowseSquare(from: URL(string: imageURL)!)
        }
    }
...
extension UIImageView{
    func getDataSongTileBrowseSquare(from url: URL, completion: @escaping (Data?, URLResponse?, Error?) -> ()) {
        URLSession.shared.dataTask(with: url, completionHandler: completion).resume()
    }
    func downloadImageSongTileBrowseSquare(from url: URL) {
        getDataSongTileBrowseSquare(from: url) { data, response, error in
