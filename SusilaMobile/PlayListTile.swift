...
class PlayListTile: UIView {
    var imageContainer = UIView(frame: CGRect(x: 0, y: 0, width: 200, height: 200))
     ...
     var imageURL = "" {
         ...
         didSet{
             if imageURL != "" {
                 //image.downloadImagePlayListTile(from: URL(string: imageURL)!)
             }
...
extension UIImageView{
    func getDataPlayListTile(from url: URL, completion: @escaping (Data?, URLResponse?, Error?) -> ()) {
        URLSession.shared.dataTask(with: url, completionHandler: completion).resume()
    }
    func downloadImagePlayListTile(from url: URL) {
        getDataPlayListTile(from: url) { data, response, error in
