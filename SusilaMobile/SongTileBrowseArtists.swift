...
class SongTileBrowseArtists: UIView {
    ...
    var imageURL = "" {
        ...
        didSet{
            //image.downloadImageSongTileBrowseArtists(from: URL(string: imageURL)!)
        }
    }
...
extension UIImageView{
    func getDataSongTileBrowseArtists(from url: URL, completion: @escaping (Data?, URLResponse?, Error?) -> ()) {
        URLSession.shared.dataTask(with: url, completionHandler: completion).resume()
    }
    func downloadImageSongTileBrowseArtists(from url: URL) {
        getDataSongTileBrowseArtists(from: url) { data, response, error in
