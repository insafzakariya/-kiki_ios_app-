...
class SongTileAlbumByArtist: UIView {
    ...
    var imageURL = "" {
        ...
        didSet{
            //image.downloadImageSongTileAlbumByArtist(from: URL(string: imageURL)!)
        }
    }
    ...
    private func commonInit() {
        ...
        imageContainer = UIView(frame: CGRect(x: 0, y: 0, width: self.frame.width , height: UIScreen.main.bounds.width/2))
        image = UIImageView(frame: CGRect(x: 0, y: 0, width:UIScreen.main.bounds.width/2-30, height: UIScreen.main.bounds.width/2-30))
        lblTitle = UILabel(frame: CGRect(x: 0, y: image.frame.height, width: image.frame.width , height: 20))
        lblDescription = UILabel(frame: CGRect(x: 0, y: image.frame.height+lblTitle.frame.height, width: image.frame.width, height: 10))
...
extension UIImageView{
    func getDataSongTileAlbumByArtist(from url: URL, completion: @escaping (Data?, URLResponse?, Error?) -> ()) {
        URLSession.shared.dataTask(with: url, completionHandler: completion).resume()
    }
    func downloadImageSongTileAlbumByArtist(from url: URL) {
        getDataSongTileAlbumByArtist(from: url) { data, response, error in
